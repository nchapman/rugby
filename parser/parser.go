...
// parseDescribeStmt parses a describe block: describe "name" do ... end
func (p *Parser) parseDescribeStmt() *ast.DescribeStmt {
	...
	if !p.curTokenIs(token.STRING) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected string after 'describe'",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	name := p.curToken.Literal
	...
	// Expect 'do'
	if !p.curTokenIs(token.DO) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'do' after describe name",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	...
	for !p.curTokenIs(token.END) && !p.curTokenIs(token.EOF) {
		...
		default:
			p.errors = append(p.errors, fmt.Sprintf("%d:%d: unexpected token %s in describe block, expected 'it', 'describe', 'before', 'after', or 'end'",
				p.curToken.Line, p.curToken.Column, p.curToken.Type))
			p.nextToken()
		}
	}
	...
	if !p.curTokenIs(token.END) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'end' to close describe block",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'end'
...
func (p *Parser) parseItStmt() *ast.ItStmt {
	...
	if !p.curTokenIs(token.STRING) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected string after 'it'",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	name := p.curToken.Literal
	...
	// Expect 'do'
	if !p.curTokenIs(token.DO) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'do' after it description",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'do'
	...
	if p.curTokenIs(token.PIPE) {
		...
		if p.curTokenIs(token.IDENT) {
			...
			if paramName != "t" {
				p.errors = append(p.errors, fmt.Sprintf(
					"%d:%d: test block parameter must be 't', got %q",
					p.curToken.Line, p.curToken.Column, paramName))
			}
			p.nextToken()
		}
		...
		if !p.curTokenIs(token.PIPE) {
			p.errors = append(p.errors, fmt.Sprintf(
				"%d:%d: expected closing '|' for block parameter",
				p.curToken.Line, p.curToken.Column))
			return nil
		}
	...
	if !p.curTokenIs(token.END) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'end' to close it block",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'end'
...
func (p *Parser) parseTestStmt() *ast.TestStmt {
	...
	// Expect string for name
	if !p.curTokenIs(token.STRING) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected string after 'test'",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	name := p.curToken.Literal
	...
	// Expect 'do'
	if !p.curTokenIs(token.DO) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'do' after test name",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'do'
	...
	if !p.curTokenIs(token.END) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'end' to close test block",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'end'
...
func (p *Parser) parseTableStmt() *ast.TableStmt {
	...
	// Expect string for name
	if !p.curTokenIs(token.STRING) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected string after 'table'",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	name := p.curToken.Literal
	...
	// Expect 'do'
	if !p.curTokenIs(token.DO) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'do' after table name",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'do'
	...
	if p.curTokenIs(token.PIPE) {
		...
		if p.curTokenIs(token.IDENT) {
			...
			if paramName != "tt" {
				p.errors = append(p.errors, fmt.Sprintf(
					"%d:%d: table block parameter must be 'tt', got %q",
					p.curToken.Line, p.curToken.Column, paramName))
			}
			p.nextToken()
		}
	...
	if !p.curTokenIs(token.END) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'end' to close table block",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'end'
...
func (p *Parser) parseBeforeStmt() *ast.BeforeStmt {
	...
	// Expect 'do'
	if !p.curTokenIs(token.DO) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'do' after 'before'",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'do'
	...
	if p.curTokenIs(token.PIPE) {
		...
		if p.curTokenIs(token.IDENT) {
			...
			if paramName != "t" {
				p.errors = append(p.errors, fmt.Sprintf(
					"%d:%d: before block parameter must be 't', got %q",
					p.curToken.Line, p.curToken.Column, paramName))
			}
			p.nextToken()
		}
	...
	if !p.curTokenIs(token.END) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'end' to close before block",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'end'
...
func (p *Parser) parseAfterStmt() *ast.AfterStmt {
	...
	// Expect 'do'
	if !p.curTokenIs(token.DO) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'do' after 'after'",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'do'
	...
	if p.curTokenIs(token.PIPE) {
		...
		if p.curTokenIs(token.IDENT) {
			...
			if paramName != "t" {
				p.errors = append(p.errors, fmt.Sprintf(
					"%d:%d: after block first parameter must be 't', got %q",
					p.curToken.Line, p.curToken.Column, paramName))
			}
			p.nextToken()
		}
	...
	if !p.curTokenIs(token.END) {
		p.errors = append(p.errors, fmt.Sprintf("%d:%d: expected 'end' to close after block",
			p.curToken.Line, p.curToken.Column))
		return nil
	}
	p.nextToken() // consume 'end'
